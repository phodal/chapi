...
open class ArkTSAstListener : ArkTSParserBaseListener() {
    fun buildParameters(
        formalParameterListContext: ArkTSParser.FormalParameterListContext?
    ): List<CodeProperty> {
        ...
        for (argCtx in formalParameterListContext!!.formalParameterArg()) {
            ...
            val typeRef = ArkTSTypeRefBuilder.build(argCtx.typeAnnotation())
    ...
    fun buildTypeAnnotation(typeAnnotation: ArkTSParser.TypeAnnotationContext?): String {
    ...
    fun processRef(typeRef: ArkTSParser.TypeRefContext?): String? {
        if (typeRef == null) return null
        return typeRef.text
    }

    fun buildMethodParameters(paramListCtx: ArkTSParser.ParameterListContext?): List<CodeProperty> {
    ...
    private fun buildRestParameter(restCtx: ArkTSParser.RestParameterContext?): CodeProperty {
        var paramType = ""
        var typeRef: CodeTypeRef? = null
        if (restCtx!!.typeAnnotation() != null) {
            paramType = buildTypeAnnotation(restCtx.typeAnnotation())
            typeRef = ArkTSTypeRefBuilder.build(restCtx.typeAnnotation())
        }
    ...
    private fun buildOptionalParameter(paramCtx: ArkTSParser.OptionalParameterContext): CodeProperty {
        var paramType = ""
        var typeRef: CodeTypeRef? = null
        if (paramCtx.typeAnnotation() != null) {
            paramType = buildTypeAnnotation(paramCtx.typeAnnotation())
            typeRef = ArkTSTypeRefBuilder.build(paramCtx.typeAnnotation())
        }
    ...
    private fun buildRequireParameter(paramCtx: ArkTSParser.RequiredParameterContext): CodeProperty {
        var paramType = ""
        var typeRef: CodeTypeRef? = null
        if (paramCtx.typeAnnotation() != null) {
            paramType = buildTypeAnnotation(paramCtx.typeAnnotation())
    ...
    fun buildAnnotation(decorator: ArkTSParser.DecoratorContext): CodeAnnotation {
