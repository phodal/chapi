...
// Switch Statement
switch_statement
    : SWITCH expression LCURLY switch_cases? RCURLY
    ;
...
switch_case
    : (case_label | default_label) statements
    | conditional_switch_case
    ;

case_label
    : attributes? CASE case_item_list COLON
    ;
...
conditional_switch_case
    : switch_if_directive_clause switch_elseif_directive_clauses? switch_else_directive_clause? HASH_ENDIF
    ;
...
switch_else_directive_clause
    : HASH_ELSE switch_cases?
    ;

// Labeled Statement
labeled_statement
    : statement_label (loop_statement | if_statement | switch_statement | do_statement)
    ;
...
if_directive_clause
    : HASH_IF compilation_condition statements?
    ;
...
compilation_condition
    ...
    | compilation_condition (compilation_condition_AND | compilation_condition_OR) compilation_condition
    ;

platform_condition
    : OS LPAREN operating_system RPAREN
    | ARCH LPAREN architecture RPAREN
    | (SWIFT | COMPILER) LPAREN (compilation_condition_GE | compilation_condition_L) swift_version RPAREN
    | CAN_IMPORT LPAREN module_name RPAREN
    | TARGET_ENVIRONMENT LPAREN environment RPAREN
    ;

swift_version
    : Decimal_digits swift_version_continuation?
    ;
...
diagnostic_statement
    : (ERROR | WARNING) LPAREN diagnostic_message RPAREN
    ;
...
generic_parameter
    : type_name (COLON (type_identifier | protocol_composition_type))?
    ;
...
conformance_requirement
    : type_identifier COLON (type_identifier | protocol_composition_type)
    ;
...
getter_setter_block
    : LCURLY (getter_clause setter_clause? | setter_clause getter_clause) RCURLY
    | code_block
    ;
...
willSet_didSet_block
    : LCURLY (willSet_clause didSet_clause? | didSet_clause willSet_clause?) RCURLY
    ;

willSet_clause
    : attributes? WILL_SET setter_name? code_block
    ;

didSet_clause
    : attributes? DID_SET setter_name? code_block
    ;
...
enum_declaration
    : attributes? access_level_modifier? (union_style_enum | raw_value_style_enum)
    ;

union_style_enum
    : INDIRECT? ENUM enum_name generic_parameter_clause? type_inheritance_clause? generic_where_clause? LCURLY union_style_enum_members? RCURLY
    ;
...
raw_value_style_enum
    : ENUM enum_name generic_parameter_clause? type_inheritance_clause generic_where_clause? LCURLY raw_value_style_enum_members RCURLY
    ;
...
struct_declaration
    : attributes? access_level_modifier? STRUCT struct_name generic_parameter_clause? type_inheritance_clause? generic_where_clause? struct_body
    ;
...
// Class Declaration
class_declaration
    : attributes? (access_level_modifier? FINAL? | FINAL access_level_modifier?) CLASS class_name generic_parameter_clause? type_inheritance_clause?
...
protocol_body
    : LCURLY protocol_members RCURLY
    ;
...
protocol_initializer_declaration
    : initializer_head generic_parameter_clause? parameter_clause (THROWS? | RETHROWS) generic_where_clause?
    ;
...
protocol_associated_type_declaration
    : attributes? access_level_modifier? ASSOCIATED_TYPE typealias_name type_inheritance_clause? typealias_assignment? generic_where_clause?
    ;

// Initializer Declaration
initializer_declaration
    : initializer_head generic_parameter_clause? parameter_clause (THROWS | RETHROWS)? generic_where_clause? initializer_body
    ;
...
extension_declaration
    : attributes? access_level_modifier? EXTENSION type_identifier type_inheritance_clause? generic_where_clause? extension_body
    ;

extension_body
    : LCURLY extension_members RCURLY
    ;
...
access_level_modifier
    : (PRIVATE | FILE_PRIVATE | INTERNAL | PUBLIC | OPEN) (LPAREN SET RPAREN)?
    ;
...
// Tuple Pattern
tuple_pattern
    : LPAREN tuple_pattern_element_list? RPAREN
    ;
...
// Attributes
attribute
    : AT attribute_name attribute_argument_clause?
    ;
...
balanced_token_punctuation
    : (DOT | COMMA | COLON | SEMI | EQUAL | AT | HASH | BACKTICK | QUESTION)
    | arrow_operator
...
unqualified_name
    : identifier (LPAREN argument_names RPAREN)?
    ;
...
array_literal
    : LBRACK array_literal_items? RBRACK
    ;
...
playground_literal
    : HASH_COLOR_LITERAL LPAREN RED COLON expression COMMA GREEN COLON expression COMMA BLUE COLON expression COMMA ALPHA COLON expression RPAREN
...
capture_list
    : LBRACK capture_list_items RBRACK
    ;
...
capture_list_item
    : capture_specifier? (identifier EQUAL? expression | self_expression)
    ;
...
initializer_suffix
    : DOT INIT (LPAREN argument_names RPAREN)?
    ;

explicit_member_suffix
    : DOT (Decimal_digits | identifier ( generic_argument_clause | LPAREN argument_names RPAREN)?)
    ;
...
function_call_argument
    : argument_name? (identifier /*optimization */ | expression | operator)
    ;
...
labeled_trailing_closure
    : identifier COLON closure_expression
    ;
...
// Tuple Type
tuple_type
    : LPAREN tuple_type_element_list? RPAREN
    ;
...
element_name
    : identifier+
    ; // TODO understand a specific thing - _
...
// Assignment Operator
assignment_operator
    : {this.isBinaryOp(_input)}? EQUAL
    ;

negate_prefix_operator
    : {this.isPrefixOp(_input)}? SUB
    ;
...
compilation_condition_L
    : {this.isOperator(_input,"<")}? LT
    ;

arrow_operator
    : {this.isOperator(_input,"->")}? SUB GT
    ;

range_operator
    : {this.isOperator(_input,"...")}? DOT DOT DOT
    ;
...
binary_operator
    : {this.isBinaryOp(_input)}? operator
    ;

prefix_operator
    : {this.isPrefixOp(_input)}? operator
    ;

postfix_operator
    : {this.isPostfixOp(_input)}? operator
    ;
...
extended_string_literal
    : Multi_line_extended_string_open Quoted_multi_line_extended_text+ Multi_line_extended_string_close
    | Single_line_extended_string_open Quoted_single_line_extended_text+ Single_line_extended_string_close
    ;
