...
MAC_CATALYST_APPLICATION_EXTENSION : 'macCatalystApplicationExtension';
...
DOT    : '.';
LCURLY : '{';
LPAREN : '(' { if(!parenthesis.isEmpty()) parenthesis.push(parenthesis.pop()+1);};
LBRACK : '[';
RCURLY : '}';
RPAREN:
...
Multi_line_extended_string_open: '#'+ '"""' -> pushMode(MultiLineExtended);

Single_line_extended_string_open: '#'+ '"' -> pushMode(SingleLineExtended);
...
fragment Multiline_quoted_text: Escaped_character | ~[\\"]+ | '"' '"'? | Escaped_newline;
